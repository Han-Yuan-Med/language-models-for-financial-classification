...
model_output_content.to_excel('Gemma_good_credit_accuracy_original.xlsx', index=False)

model_output_content = pd.read_excel('Gemma_good_credit_accuracy_original.xlsx')
good_correct_id = []
good_wrong_id = []
...
model_output_content = pd.DataFrame(model_output_content, columns=['Model input', 'Model output'])
model_output_content.to_excel('Gemma_bad_credit_accuracy_original.xlsx', index=False)

model_output_content = pd.read_excel('Gemma_bad_credit_accuracy_original.xlsx')
bad_correct_id = []
bad_wrong_id = []
for i in range(len(model_output_content)):
    print(model_output_content.iloc[i, 1].split('\n')[0])
